# 工作流程手册 SOP

这个目录记录启效智联汽车销售线索系统的标准操作流程。第一版以“小白能照着做”为标准，先覆盖开发和业务原型的关键流程。

## 已建立 SOP

- [开发环境搭建](./development-setup.md)
- [项目长期开发规范](./project-governance.md)
- [页面开发流程](./page-development.md)
- [个人业务驾驶舱流程](./personal-dashboard.md)
- [客户项目管理流程](./customer-project-management.md)
- [报价系统流程](./quotation-system.md)
- [线索导入流程](./lead-import.md)
- [线索清洗与去重](./lead-cleaning.md)
- [线索搜索、状态流转与跟进](./lead-follow-up.md)
- [城市、品牌、车型智能选择](./smart-selection.md)
- [供应商管理流程](./supplier-management.md)
- [成交开拓基地流程](./deal-base.md)
- [交付批次流程](./delivery-batch.md)
- [发布前检查](./release-checklist.md)

## 维护规则

- 每新增一个核心业务模块，同步新增或更新 SOP。
- 每个 SOP 都要包含使用场景、操作步骤、注意事项、常见问题。
- 真实流程变化后，先更新系统，再更新 SOP，最后运行发布前检查。
